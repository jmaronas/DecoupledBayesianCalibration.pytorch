...
var=`python ../main_predictive_inference_withLR.py --model_net $model --data_dir ./data/ --dataset $database --n_gpu 0 --valid_test test --MCsamples $mcvalid --n_layers $layers --layer_dim $neuron --model_dir $model_dir"/"$mcsamplestrain"MC_"$epochs"eps_DKLSF"$dklsf"/models/BNN.pth.tar" |  awk 'BEGIN{}{print $11","$15","substr($18,1,length($18)-1)}'`
