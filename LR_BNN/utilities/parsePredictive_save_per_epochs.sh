...
echo "Neurons per layer,layers,MonteCarlo,Epochs,DKLSF,Save After,ECE15,Accuracy,ECE15Valid,MCValid" >   "ParsePredictive"$database$model".txt"
...
cat $dirPredictive"/"$file  | grep "Best" | awk -v dklsf="$dklsf" -v var="$file" -v mc_samples="$mc_samples" -v epochs="$epochs" -v num_layers="$number_layers" -v neurons_per_layer="$neurons_per_layer" -v save_after=$save_after 'BEGIN{}{print neurons_per_layer","num_layers","mc_samples","epochs","dklsf","save_after", , ,"$7","$10} ' >> "ParsePredictive"$database$model".txt"
